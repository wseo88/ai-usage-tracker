...
# Current pricing per 1M tokens (used as fallback if pricing_snapshots not available)
FALLBACK_PRICING: dict[str, dict[str, float]] = {
    ...
    "claude-3-haiku-20240307": {"input": 0.25, "output": 1.25, "cache_write": 0.30, "cache_read": 0.025},
}
...
def _get_pricing_from_db() -> dict[str, dict[str, float]]:
    ...
    try:
        ...
        resp = (
            ...
            .select("model, input_price_per_m, output_price_per_m, cache_write_price_per_m, cache_read_price_per_m")
...
def _flatten_buckets(buckets: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for bucket in buckets:
        ...
        for result in results:
            ...
            input_tokens = token_counts.get("uncached_input", 0) + token_counts.get("cached_input", 0)
            output_tokens = token_counts.get("output", 0)
            cache_write_tokens = token_counts.get("cache_creation", 0)
            cache_read_tokens = token_counts.get("cached_input", 0) if "cached_input" in token_counts else 0
...
def sync(days: int = DEFAULT_DAYS) -> dict[str, Any]:
    ...
    provider_resp = db.table("providers").select("id", "enabled").eq("slug", "anthropic").execute()
    if not provider_resp.data:
        raise AnthropicSyncError(
            "Anthropic provider not found in database. Run seed.sql first."
        )
    ...
    # ── Load pricing from DB (fallback to hardcoded) ──────────
    ...
    logger.info(
        "Anthropic sync complete: %d records for %d days",
        records_inserted,
        days,
    )
